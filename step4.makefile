...
jobs/step4-simulation.txt.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	cat $(LD) | tail -n+2  | awk '($$3 - $$2) > 1e6 { print NR }' | awk 'BEGIN{ srand(9689) } { print rand() FS $$1 }' | sort -k1g | head -n10 | awk '{ print $$2 }' | awk '{ hq = 0.17; split("10 100", ngenes); split("1 10", ncgenes); for(g = 1; g <= 2; ++g) { ng = ngenes[g]; nc = ncgenes[g]; for(sr = 0.3; sr <= 0.6; sr += 0.3) for(qq = 2; qq < 5; qq+=2) for(hm = 0.0; hm <= 0.3; hm += 0.05) for(hu = 0.0; hu <= 0.5; hu += 0.1) for(nu = 1; nu <= 3; ++nu) print "./make.simulation.R " $$1 FS sr FS ng FS nc FS qq FS nu FS hq FS hm FS hu FS ("./simulation/" ng "_" nc "/" (ng "_" nc "_" sr "_" nu "_" qq "_" hq "_" hm "_" hu) "_" $$1 ".txt.gz") } }' | awk 'system("! [ -f " $$NF " ]") == 0' | gzip > $@
