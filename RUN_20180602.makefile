...
run-jobs: $(foreach gwas, $(GWAS), jobs/20180602-$(gwas).txt.gz)

run-null: $(foreach gwas, $(GWAS), jobs/20180602-$(gwas).null.gz)

long: $(foreach gwas, $(GWAS), jobs/20180602-$(gwas).long.gz)

jobs/20180602-%.txt.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(NBLOCKS) | awk '{ g = 4; e = 2; print "./make.cammel-$*-joint.R" FS $$1 FS ("1e"g) FS ("1e-"e) FS "rosmap-mult:mayo-mult:geuvadis-mult" FS ("result/20180602/obs/$*/" $$1) }' | gzip >> $@
...
jobs/20180602-%.null.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(NBLOCKS) | awk '{ g = 4; e = 2; print "./make.cammel-$*-joint.R" FS $$1 FS ("1e"g) FS ("1e-"e) FS "rosmap-mult:mayo-mult:geuvadis-mult" FS ("result/20180602/null/$*/" $$1) FS "TRUE" }' | gzip >> $@
...
jobs/20180602-igap.long.gz: jobs/20180602-igap.txt.gz
...
################################################################
table: $(foreach _trait, ukbb_ad_mat ukbb_ad_pat ukbb_moodswings ukbb_neuroticism pgc_scz pgc_mdd pgc_asd pgc_adhd pgc_ocd pgc_ptsd_civ_ea pgc_ptsd_mil_ea pgc_ptsd_all_ea igap_ad, result/20180602/obs/gene-$(_trait).txt.gz)
