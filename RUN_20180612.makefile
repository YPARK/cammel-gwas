...
run-jobs: $(foreach gwas, $(GWAS), jobs/20180612-$(gwas).txt.gz)

run-null: $(foreach gwas, $(GWAS), jobs/20180612-$(gwas).null.gz)

long: $(foreach gwas, $(GWAS), jobs/20180612-$(gwas).long.gz)

jobs/20180612-%.txt.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(NBLOCKS) | awk '{ g = 2; e = 2; print "./make.cammel-$*-joint.R" FS $$1 FS ("1e"g) FS ("1e-"e) FS "rosmap-mult:mayo-mult:geuvadis-mult" FS ("result/20180612/obs/$*/" $$1) }' | gzip >> $@
	qsub -P compbio_lab -o /dev/null -binding linear:1 -cwd -V -l h_vmem=2g -l h_rt=12:00:00 -b y -j y -N cammel_$* -t 1-$$(zcat $@ | wc -l) ./run_jobs.sh $@

jobs/20180612-%.null.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(NBLOCKS) | awk '{ g = 2; e = 2; print "./make.cammel-$*-joint.R" FS $$1 FS ("1e"g) FS ("1e-"e) FS "rosmap-mult:mayo-mult:geuvadis-mult" FS ("result/20180612/null/$*/" $$1) FS "TRUE" }' | gzip >> $@
	qsub -P compbio_lab -o /dev/null -binding linear:1 -cwd -V -l h_vmem=2g -l h_rt=12:00:00 -b y -j y -N null_$* -t 1-$$(zcat $@ | wc -l) ./run_jobs.sh $@

jobs/20180612-igap.long.gz: jobs/20180612-igap.txt.gz
...
jobs/20180612-pgc.long.gz: jobs/20180612-pgc.txt.gz
...
jobs/20180612-ukbb.long.gz: jobs/20180612-ukbb.txt.gz
...
result/20180612/obs/gene-%.txt.gz: make.cammel-gene-table.R
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	./run.sh Rscript --vanilla $< result/20180612/obs/$(shell echo $* | awk -F'_' '{ print $$1 "/ " $$0 }') $@
